...
namespace NinjaTrader.NinjaScript.ChartStyles
{
	public class CandleStyle : ChartStyle
	{
		...
		public override object Clone()
		{
			...
			// after a NinjaScript compile base.Clone() creates the clone from the newly loaded assembly, so the cast below fails
			// on pre-compile instances even though the clone is valid and all properties were already copied [NT-47733; AR 20260723]
			if (clone is not CandleStyle ret)
				return clone ?? new CandleStyle();
