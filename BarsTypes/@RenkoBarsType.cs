	...
	public class RenkoBarsType : BarsType
	{
		...
		protected override void OnDataPoint(Bars bars, double open, double high, double low, double close, DateTime time, long volume, bool isBar, double bid, double ask)
		{
			...
			if (close.ApproxCompare(renkoHigh) >= 0)
			{
				...
				{
					AddBar(bars, renkoHigh - offset, Math.Max(renkoHigh - offset, renkoHigh), Math.Min(renkoHigh - offset, renkoHigh), renkoHigh, time < barTime ? barTime : time, 0);
					renkoLow	= renkoHigh - 2.0 * offset;
					renkoHigh += offset;
				}

				// Add final partial bar
				AddBar(bars, renkoHigh - offset, Math.Max(renkoHigh - offset, close), Math.Min(renkoHigh - offset, close), close, time < barTime ? barTime : time, volume);
			}
			else
				if (close.ApproxCompare(renkoLow) <= 0)
				{
					...
					{
						AddBar(bars, renkoLow + offset, Math.Max(renkoLow + offset, renkoLow), Math.Min(renkoLow + offset, renkoLow), renkoLow, time < barTime ? barTime : time, 0);
						renkoHigh	= renkoLow + 2.0 * offset;
						renkoLow	-= offset;
					}

					// Add final partial bar
					AddBar(bars, renkoLow + offset, Math.Max(renkoLow + offset, close), Math.Min(renkoLow + offset, close), close, time < barTime ? barTime : time, volume);
				}
				else
					UpdateBar(bars, close, close, close, time < barTime ? barTime : time, volume);
