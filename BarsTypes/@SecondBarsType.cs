...
namespace NinjaTrader.NinjaScript.BarsTypes
{
	public class SecondBarsType : BarsType
	{
		...
		public override void ApplyDefaultValue(BarsPeriod period) => period.Value = 30;
		...
		public override int GetInitialLookBackDays(BarsPeriod period, TradingHours tradingHours, int barsBack)
			=> (int) Math.Max(1, Math.Ceiling(barsBack / Math.Max(1, 8.0 * 60 * 60 / period.Value)) * 7.0 / 5.0);

		public override double GetPercentComplete(Bars bars, DateTime now)
			=> now <= bars.LastBarTime ? 1.0 - bars.LastBarTime.Subtract(now).TotalSeconds / bars.BarsPeriod.Value : 1;
		...
		protected override void OnStateChange()
		{
			...
			{
				Name = $"{string.Format(Core.Globals.GeneralOptions.CurrentCulture, Custom.Resource.DataBarsTypeSecond, BarsPeriod.Value)}{(BarsPeriod.MarketDataType != MarketDataType.Last ? $" - {Core.Globals.ToLocalizedObject(BarsPeriod.MarketDataType, Core.Globals.GeneralOptions.CurrentUICulture)}" : string.Empty)}";
