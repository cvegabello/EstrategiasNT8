...
namespace NinjaTrader.NinjaScript.ImportTypes
{
	...
	public class TickDataImportType : ImportType
	{
		...
		protected override void OnNextDataPoint()
		{
			...
			while (true)
			{
				...
				try
				{
					...
					Volume 	= isCryptoCurrency ? Core.Globals.FromCryptocurrencyVolume(Convert.ToDouble(matches[3].Value.Trim(quotes).Trim(), cultureInfo)) : Convert.ToInt64(matches[3].Value.Trim(quotes).Trim(), cultureInfo);
		...
		protected override void OnNextInstrument()
		{
			...
			{
				FileInfo fileInfo		= new(FileNames[++currentInstrumentIdx]);
		...
		protected override void OnStateChange()
		{
			switch (State)
			{
				...
				case State.Configure:
				{
					...
					if (dialog.FileNames is not { Length: > 0 }) 
					{
						SetState(State.Terminated);
						return;
					}
